...
exp(decLM$time.series[,1])#seasnal components extracted
unique(exp(decLM$time.series[,1]))#Monthly indices


#Forecasting using lowess smoother
fore<-forecast(dec,h=10,method="naive") #h -> number of time periods(depends on frequency of time series)
accuracy(fore,x =window(ml,start=c(1972,2),end=c(1972,11)))
...
hol<-holt(mlTr,h=10) #forecasting for 5 periods
accuracy(hol,x =window(ml,start=c(1972,2),end=c(1972,11)))

#Fitting holts's model with exponential trend
hole<-holt(mlTr,h=10,exponential = T) #forecasting for 5 periods
accuracy(hole,x =window(ml,start=c(1972,2),end=c(1972,11)))
